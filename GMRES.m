%% 操作量の変化量の計算（C/GMRES法）
function du_new = GMRES( x_current, du, u, d, T, sys, cgmres )
    ...
    Fxt = CalcF( x_current + ( dx * cgmres.ht ), u, d, T, sys, cgmres ); % 函数Fの状態微分
    
    F = CalcF( x_current, u, d, T, sys, cgmres ); % 函数F
    
    Right = - cgmres.zeta * F - ( ( Fxt - F ) / cgmres.ht ); % 既知項を右辺にまとめる
    
    Fuxt = CalcF( x_current + ( dx * cgmres.ht ), u + ( du * cgmres.ht ), d, T, sys, cgmres ); % 函数Fの入力，状態微分
    ...
    for cnt = 1:k
        Fuxt = CalcF( x_current + ( dx * cgmres.ht ), u + ( v(:,cnt) * cgmres.ht ), d, T, sys, cgmres );
